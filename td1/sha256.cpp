/*
*   SHA-256 implementation.
*
*   Copyright (c) 2010,2014,2018,2021,2022 Willem H. Hengeveld <itsme@xs4all.nl>
*
*   This code is released under the MIT license.
*
*   This is a plain C implementation of the SHA-256 algorithm.
*   It has been written to be as clear as possible, with a focus on
*   the algorithm itself, and not on performance.
*
*   The most recent version of this code can be found at:
*   https://github.com/whe-its-me/sha256
*
*/
#include <string.h>
#include <stdio.h>
#include "sha256.h"

// The SHA-256 algorithm uses a number of logical functions and constants.
// These are defined in the FIPS-180-4 standard.
//
// The logical functions are:
//
// CH(x,y,z) = (x & y) ^ (~x & z)
// MAJ(x,y,z) = (x & y) ^ (x & z) ^ (y & z)
//
// BSIG0(x) = ROTR(2,x) ^ ROTR(13,x) ^ ROTR(22,x)
// BSIG1(x) = ROTR(6,x) ^ ROTR(11,x) ^ ROTR(25,x)
//
// SSIG0(x) = ROTR(7,x) ^ ROTR(18,x) ^ SHR(3,x)
// SSIG1(x) = ROTR(17,x) ^ ROTR(19,x) ^ SHR(10,x)
//
// where ROTR(n,x) is the circular right shift of x by n bits.
// and SHR(n,x) is the right shift of x by n bits.
//
// The constants are the first 32 bits of the fractional parts of the
// cube roots of the first 64 prime numbers.
//
// The algorithm operates on 512-bit blocks, and produces a 256-bit hash.
// The message is padded to a multiple of 512 bits.
// The padding consists of a single 1 bit, followed by a number of 0 bits,
// and finally the 64-bit message length.
//
// The algorithm maintains a 256-bit state, which is initialized with the
// first 32 bits of the fractional parts of the square roots of the first 8
// prime numbers.
//
// The state is updated for each block of the message.
// The update consists of 64 rounds.
// Each round uses a message schedule word, a constant, and the current state.
// The message schedule is derived from the 512-bit message block.
//
// The final hash is the state after processing all blocks.

#ifdef SHA256_USE_64BIT
// 64-bit implementation
// this is mainly for demonstration purposes.
// it is slower than the 32-bit version.

#define ROTR(n, x) (((x) >> (n)) | ((x) << (64 - (n))))
#define SHR(n, x) ((x) >> (n))

#define CH(x, y, z) (((x) & (y)) ^ (~(x) & (z)))
#define MAJ(x, y, z) (((x) & (y)) ^ ((x) & (z)) ^ ((y) & (z)))

#define BSIG0(x) (ROTR(2, x) ^ ROTR(13, x) ^ ROTR(22, x))
#define BSIG1(x) (ROTR(6, x) ^ ROTR(11, x) ^ ROTR(25, x))

#define SSIG0(x) (ROTR(7, x) ^ ROTR(18, x) ^ SHR(3, x))
#define SSIG1(x) (ROTR(17, x) ^ ROTR(19, x) ^ SHR(10, x))

#else
// 32-bit implementation

#define ROTR(n, x) (((x) >> (n)) | ((x) << (32 - (n))))
#define SHR(n, x) ((x) >> (n))

#define CH(x, y, z) (((x) & (y)) ^ (~(x) & (z)))
#define MAJ(x, y, z) (((x) & (y)) ^ ((x) & (z)) ^ ((y) & (z)))

#define BSIG0(x) (ROTR(2, x) ^ ROTR(13, x) ^ ROTR(22, x))
#define BSIG1(x) (ROTR(6, x) ^ ROTR(11, x) ^ ROTR(25, x))

#define SSIG0(x) (ROTR(7, x) ^ ROTR(18, x) ^ SHR(3, x))
#define SSIG1(x) (ROTR(17, x) ^ ROTR(19, x) ^ SHR(10, x))

#endif

// The first 32 bits of the fractional parts of the square roots of the first 8 primes
static const sha256_word H_0[8] = {
    0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
    0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19
};

// The first 32 bits of the fractional parts of the cube roots of the first 64 primes
static const sha256_word K[64] = {
    0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
    0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
    0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
    0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
    0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
    0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
    0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
    0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2
};

// reverses the byte order of a 32-bit word
static uint32_t reverse(uint32_t x)
{
    return ((x & 0xff) << 24) | (((x >> 8) & 0xff) << 16) | (((x >> 16) & 0xff) << 8) | ((x >> 24) & 0xff);
}

// process a single 512-bit block
static void sha256_transform(sha256_context *ctx)
{
    sha256_word a, b, c, d, e, f, g, h;
    sha256_word T1, T2;
    sha256_word W[64];
    int t;

#ifdef SHA256_USE_64BIT
    // the 64-bit version is not yet optimized
    for (t = 0; t < 16; t++) {
        W[t] = ctx->W[t];
    }
#else
    // the 32-bit version is optimized
    // it uses the fact that the message schedule can be calculated on the fly
    // and that the message block is in big-endian format
    //
    // copy the 16 words from the context to the W array
    // and reverse the byte order
    for (t = 0; t < 16; t++) {
        W[t] = reverse(ctx->W[t]);
    }
#endif

    for (t = 16; t < 64; t++) {
        W[t] = SSIG1(W[t - 2]) + W[t - 7] + SSIG0(W[t - 15]) + W[t - 16];
    }

    a = ctx->h[0];
    b = ctx->h[1];
    c = ctx->h[2];
    d = ctx->h[3];
    e = ctx->h[4];
    f = ctx->h[5];
    g = ctx->h[6];
    h = ctx->h[7];

    for (t = 0; t < 64; t++) {
        T1 = h + BSIG1(e) + CH(e, f, g) + K[t] + W[t];
        T2 = BSIG0(a) + MAJ(a, b, c);
        h = g;
        g = f;
        f = e;
        e = d + T1;
        d = c;
        c = b;
        b = a;
        a = T1 + T2;
    }

    ctx->h[0] += a;
    ctx->h[1] += b;
    ctx->h[2] += c;
    ctx->h[3] += d;
    ctx->h[4] += e;
    ctx->h[5] += f;
    ctx->h[6] += g;
    ctx->h[7] += h;
}

void sha256_init(sha256_context *ctx)
{
    int i;
    for (i = 0; i < 8; i++) {
        ctx->h[i] = H_0[i];
    }
    ctx->size = 0;
    ctx->buflen = 0;
}

void sha256_update(sha256_context *ctx, const uint8_t *data, size_t len)
{
    size_t i;
    for (i = 0; i < len; i++) {
        ((uint8_t*)ctx->W)[ctx->buflen++] = data[i];
        if (ctx->buflen == 64) {
            sha256_transform(ctx);
            ctx->buflen = 0;
        }
    }
    ctx->size += len;
}

void sha256_final(sha256_context *ctx, uint8_t *hash)
{
    uint8_t *p = (uint8_t*)ctx->W;
    int i;

    // pad the message
    // add a single '1' bit
    p[ctx->buflen++] = 0x80;

    // add '0' bits until the message length is 56 bytes
    if (ctx->buflen > 56) {
        while (ctx->buflen < 64) {
            p[ctx->buflen++] = 0;
        }
        sha256_transform(ctx);
        ctx->buflen = 0;
    }
    while (ctx->buflen < 56) {
        p[ctx->buflen++] = 0;
    }

    // add the 64-bit message length
    uint64_t size = ctx->size * 8;
    for (i = 0; i < 8; i++) {
        p[56 + i] = (size >> (56 - i * 8)) & 0xff;
    }

    sha256_transform(ctx);

    // copy the hash to the output buffer
    for (i = 0; i < 8; i++) {
        hash[i * 4 + 0] = (ctx->h[i] >> 24) & 0xff;
        hash[i * 4 + 1] = (ctx->h[i] >> 16) & 0xff;
        hash[i * 4 + 2] = (ctx->h[i] >> 8) & 0xff;
        hash[i * 4 + 3] = (ctx->h[i] >> 0) & 0xff;
    }
}

void sha256_hash(const uint8_t *data, size_t len, uint8_t *hash)
{
    sha256_context ctx;
    sha256_init(&ctx);
    sha256_update(&ctx, data, len);
    sha256_final(&ctx, hash);
}

std::string sha256(const std::string& str)
{
    uint8_t hash[32];
    sha256_hash((const uint8_t*)str.c_str(), str.size(), hash);
    char hex[65];
    for (int i = 0; i < 32; i++) {
        sprintf(hex + i * 2, "%02x", hash[i]);
    }
    hex[64] = 0;
    return std::string(hex);
}
